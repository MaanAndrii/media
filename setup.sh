...
gather_config() {
  ...
  CFG_HOME="$(getent passwd "$CFG_USER" 2>/dev/null | cut -d: -f6 || echo "/home/$CFG_USER")"
...
install_packages() {
  ...
  # Зберегти конфіг для update-all.sh (printf %q — безпечне цитування для source)
  printf '# Генерується setup.sh автоматично\nSERVICE_USER=%q\nNEWSMON_DIR=%q\nWM_DIR=%q\nWRITER_DIR=%q\n' \
    "$CFG_USER" "$CFG_NEWSMON_DIR" "$CFG_WM_DIR" "$CFG_WRITER_DIR" \
...
setup_sysapi() {
  ...
  # Sudoers — дозволяє sysapi керувати сервісами без пароля
  local sudoers_f="/etc/sudoers.d/sysapi"
  local tmp_s; tmp_s=$(mktemp)
  chmod 600 "$tmp_s"
  cat > "$tmp_s" <<EOF
# Pi Media sysapi — passwordless service management (generated by setup.sh)
$CFG_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart newsmon
$CFG_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart watermarker
$CFG_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart sysapi
$CFG_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart php*
$CFG_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl reload nginx
$CFG_USER ALL=(ALL) NOPASSWD: /usr/bin/bash
$CFG_USER ALL=(ALL) NOPASSWD: /usr/bin/install
$CFG_USER ALL=(www-data) NOPASSWD: /usr/bin/git
EOF
  if sudo visudo -c -f "$tmp_s" 2>/dev/null; then
    sudo install -m 440 "$tmp_s" "$sudoers_f"
    ok "sudoers: $sudoers_f"
  else
    warn "visudo перевірка не пройшла — додай sudoers вручну: sudo visudo -f $sudoers_f"
  fi
  rm -f "$tmp_s"

  # sysapi.env — пишемо повністю (уникаємо sed-ін'єкції через символи в паролі)
  ...
  run sudo chmod 600 "$envfile"
  sudo chown "$CFG_USER" "$envfile" 2>/dev/null || true
  ok "sysapi.env збережено: $envfile"
