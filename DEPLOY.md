...
sudo apt update
sudo apt upgrade -y
sudo apt install -y git nginx php-fpm php-cli php-curl php-sqlite3 php-mbstring \
...
# ОБОВ'ЯЗКОВО на Raspberry Pi: streamlit-sortables від piwheels (ARM)
# йде без зібраного фронтенду frontend/build і застосунок падає при
# завантаженні фото. Ставимо примусово wheel з PyPI в обхід piwheels:
PIP_CONFIG_FILE=/dev/null pip install --no-cache-dir --only-binary=:all: \
  streamlit-sortables==0.3.1

# перевірка — має існувати index.html і static/
ls .venv/lib/python3.13/site-packages/streamlit_sortables/frontend/build
deactivate
```

> Чому так: `/etc/pip.conf` на Raspberry Pi OS додає `extra-index-url`
> piwheels. piwheels пересобирає пакет для ARM зі sdist, у якому немає
> React-збірки. `PIP_CONFIG_FILE=/dev/null` повністю ігнорує цей конфіг,
> тож pip бере коректний `py3-none-any` wheel з PyPI. `--no-cache-dir`
> не дає підсунути раніше закешований биткий wheel.
...
Конфіг розрахований на сокет `php8.4-fpm.sock`. **Не правте версію
наосліп** — звірте з тим, що реально існує:

```bash
ls /run/php/
```

Якщо там, наприклад, `php8.3-fpm.sock` замість `php8.4-fpm.sock` —
підставте свою версію в конфіг:

```bash
sudo sed -i 's/php8\.[0-9]*-fpm.sock/php8.3-fpm.sock/' /etc/nginx/sites-available/pi-services.conf
grep fastcgi_pass /etc/nginx/sites-available/pi-services.conf   # перевірка
```

> Симптом неправильного сокета: статика (`/public/newswriter.html`)
> відкривається, а PHP-сторінки дають **502 Bad Gateway**, і в
> `/var/log/nginx/error.log` видно
> `connect() to unix:/run/php/...sock failed (2: No such file or directory)`.
...
> Перші чотири пункти — реальні граблі першого розгортання на Pi 5,
> у порядку, в якому вони зазвичай зринають.

### A. Watermarker падає при завантаженні фото: `No such component directory: '.../streamlit_sortables/frontend/build'`

piwheels віддав ARM-wheel `streamlit-sortables` без React-збірки.
Перевір і перевстанови з PyPI:

```bash
cd /home/maan/watermarker-pro && source .venv/bin/activate
ls .venv/lib/python3.13/site-packages/streamlit_sortables/   # frontend відсутній?
pip uninstall -y streamlit-sortables
PIP_CONFIG_FILE=/dev/null pip install --no-cache-dir --only-binary=:all: \
  streamlit-sortables==0.3.1
ls .venv/lib/python3.13/site-packages/streamlit_sortables/frontend/build  # має бути index.html + static
deactivate && sudo systemctl restart watermarker
```

Помилка з'являється саме при завантаженні фото (а не на старті) тому,
що імпорт `sort_items` лежить у блоці, який виконується лише за наявності
файлу — Streamlit прогонить скрипт згори щоразу.

### B. AI News Writer: PHP-сторінки дають 502 Bad Gateway

nginx стукає не в той сокет php-fpm. Звір реальний сокет із конфігом:

```bash
ls /run/php/
grep fastcgi_pass /etc/nginx/sites-available/pi-services.conf
sudo tail -n 5 /var/log/nginx/error.log   # покаже шлях, якого бракує
```

Виправ версію в `fastcgi_pass` під реальну і `sudo systemctl reload nginx`.
Ознака: статика відкривається, PHP — ні.

### C. AI News Writer: 404 на /api/... (експорт, налаштування, генерація)

Застосунок маршрутизує `/api/*` через фронт-контролер `index.php`.
Якщо в `location /` стоїть `try_files $uri $uri/ =404;` замість
`/index.php`, усі «гарні» URL дають 404. У `pi-services.conf` має бути:
...
### D. AI News Writer: HTTP 500 при генерації (`POST /api/job_submit`)

job_submit пише завдання в SQLite і запускає воркер через `exec()`.
Три можливі причини:

```bash
# 1) немає SQLite-розширення PHP (найчастіше)
php -m | grep -i sqlite || sudo apt install -y php8.4-sqlite3

# 2) exec() заборонено — воркер не стартує
grep -rn 'disable_functions' /etc/php/8.4/fpm/   # серед значень не має бути exec/shell_exec

# 3) воркер не бачить php-cli від імені www-data
sudo -u www-data php -r 'echo trim(shell_exec("which php"));'

sudo systemctl restart php8.4-fpm
```

Точний текст помилки — у DevTools браузера (Network → job_submit →
Response): застосунок повертає короткий JSON `{"error":"..."}`.
